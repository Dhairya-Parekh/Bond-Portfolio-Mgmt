...
def get_returns_df():
    ...
    cols_to_exclude = ['Date'] + [column for column in train_closing_prices.columns if column.startswith('Paid Coupon')]
    for column in cols_to_exclude:
        train_returns_df[column] = train_closing_prices[column]
        test_returns_df[column] = test_closing_prices[column]
    for column in train_closing_prices.columns.difference(cols_to_exclude):
...
def evaluate_portfolio(returns, coupouns, weights):
    """
    ...
    # Add coupouns to portfolio value
    portfolio_values += (coupouns * weights).sum(axis=1)
...
class RLWeights:
    def __init__(self):
        self.env = None
        policy_dir = os.path.join(os.getcwd(), '..', f'Policy_{FITTING_PERIOD}_{HOLDING_PERIOD}_final')
...
if __name__ == '__main__':
    ...
    for name, strategy in strategies.items():
        ...
        for X, Y in zip(train_X, train_Y):
            returns_X = X.iloc[:, (X.shape[1]+1)//2:].values
            weights = strategy.get_weights(returns_X)
            returns_Y = Y.iloc[:, (Y.shape[1]+1)//2:].values
            coupouns_Y = Y.iloc[:, 1:(Y.shape[1]+1)//2].values
            # Evaluate portfolio
            portfolio_metric = evaluate_portfolio(returns_Y, coupouns_Y, weights)
